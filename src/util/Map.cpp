...
Map::Map(const std::vector<double> &maps_x, const std::vector<double> &maps_y, const std::vector<double> &maps_s) :
...
Trajectory Map::getXY(const Trajectory &sdTrajectory) const {
  ...
  for (int i = 0; i < sdTrajectory.size(); i++) {
    const std::vector<double> &sd = sdTrajectory.at(i);
    const std::vector<double> &xy = getXY(sd[0], sd[1]);
